<template>
  <div>
    <cell :title="item.name" 
      ...
      is-link 
      v-for="item in items"
      v-link="{name:'bszn.detail.show', params:{id: item.code} }"
      @click ="updateTitle(item.name)"
      ></cell>
      <toast :show.sync="noResult" :time="1000" type="text">没有查询到相关办事指南</toast>
      <box gap="10px 10px">
        <x-button 
          type="primary"
...
export default {
  ...
  created () {
    this.updatePageTitle('办事指南' + (this.formData.department ? '-' + this.formData.department.name : ''))
    this.retrieveData()
  }
}
</script>
